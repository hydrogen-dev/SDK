...
public class AuthApiClient extends ApiClient {
    ...
    public String createClientCredentialReturn(String clientId, String clientSecret) throws ApiException {
        Request request = createRequest(createHttpUrl(CLIENT_CREDENTIAL), clientId, clientSecret);
        String accessToken = getAuthRequestAccessToken(request);
        return accessToken;
    }
    ...
    public String createPasswordCredentialReturn(String clientId, String clientSecret, String userName, String password) throws ApiException {
        Request request = createRequest(createHttpUrl(PASSWORD, userName, password), clientId, clientSecret);
        String accessToken = getAuthRequestAccessToken(request);
        return accessToken;
    }
    ...
    private String getAuthRequestAccessToken(Request request) throws ApiException {
        ...
        try {
            ...
            if (!response.isSuccessful()) {
                Object o = new Gson().fromJson(response.body().string(), Map.class).get(MESSAGE);
                throw new ApiException(o!=null? o.toString() : "Exception in getting auth token " + request );
            }
