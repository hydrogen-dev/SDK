...
class TransactionCode implements ModelInterface, ArrayAccess
{
    ...
    const IS_BUY_TRUE = 'true';
    const IS_BUY_FALSE = 'false';
    const IS_BUY_NULL = 'null';
    const IS_DEPOSIT_TRUE = 'true';
    const IS_DEPOSIT_FALSE = 'false';
    const IS_DEPOSIT_NULL = 'null';
     ...
     * @return string[]
     */
    public function getIsBuyAllowableValues()
    {
        return [
            self::IS_BUY_TRUE,
            self::IS_BUY_FALSE,
     ...
     * @return string[]
     */
    public function getIsDepositAllowableValues()
    {
        return [
            self::IS_DEPOSIT_TRUE,
            self::IS_DEPOSIT_FALSE,
            self::IS_DEPOSIT_NULL,
        ];
    }
    ...
    public function listInvalidProperties()
    {
        ...
        $allowedValues = $this->getIsBuyAllowableValues();
        if (!is_null($this->container['is_buy']) && !in_array($this->container['is_buy'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value for 'is_buy', must be one of '%s'",
                implode("', '", $allowedValues)
            );
        }

        $allowedValues = $this->getIsDepositAllowableValues();
        if (!is_null($this->container['is_deposit']) && !in_array($this->container['is_deposit'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value for 'is_deposit', must be one of '%s'",
                implode("', '", $allowedValues)
            );
        }

        $allowedValues = $this->getIsFeeAllowableValues();
        if (!is_null($this->container['is_fee']) && !in_array($this->container['is_fee'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value for 'is_fee', must be one of '%s'",
                implode("', '", $allowedValues)
            );
        }

        $allowedValues = $this->getIsTransferAllowableValues();
        if (!is_null($this->container['is_transfer']) && !in_array($this->container['is_transfer'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value for 'is_transfer', must be one of '%s'",
                implode("', '", $allowedValues)
            );
        }
    ...
    {
        $allowedValues = $this->getIsBuyAllowableValues();
        if (!is_null($is_buy) && !in_array($is_buy, $allowedValues, true)) {
     ...
     * Sets is_deposit
     *
     * @param string $is_deposit is_deposit
     *
     * @return $this
     */
    ...
    {
        $allowedValues = $this->getIsDepositAllowableValues();
        if (!is_null($is_deposit) && !in_array($is_deposit, $allowedValues, true)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value for 'is_deposit', must be one of '%s'",
                    implode("', '", $allowedValues)
    ...
    {
        $allowedValues = $this->getIsFeeAllowableValues();
        if (!is_null($is_fee) && !in_array($is_fee, $allowedValues, true)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value for 'is_fee', must be one of '%s'",
                    implode("', '", $allowedValues)
    ...
    {
        $allowedValues = $this->getIsTransferAllowableValues();
        if (!is_null($is_transfer) && !in_array($is_transfer, $allowedValues, true)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value for 'is_transfer', must be one of '%s'",
                    implode("', '", $allowedValues)
