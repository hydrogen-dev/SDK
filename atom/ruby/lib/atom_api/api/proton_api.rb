...
module AtomApi
  class ProtonApi
    ...
    def cash_flow_analysis(payload, opts = {})
      data, _status_code, _headers = cash_flow_analysis_with_http_info(payload, opts)
      data
    end
    ...
    # @param payload 
    # @param [Hash] opts the optional parameters
    # @return [Array<(CashFlowAnalysisResponse, Fixnum, Hash)>] CashFlowAnalysisResponse data, response status code and response headers
    def cash_flow_analysis_with_http_info(payload, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ProtonApi.cash_flow_analysis ...'
      end
      # verify the required parameter 'payload' is set
      if @api_client.config.client_side_validation && payload.nil?
        fail ArgumentError, "Missing the required parameter 'payload' when calling ProtonApi.cash_flow_analysis"
      end
      # resource path
      local_var_path = '/proton/v1/cash_flow_analysis'
      ...
      data, status_code, headers = @api_client.call_api(:POST, local_var_path,
        ...
        :body => post_body,
        :auth_names => auth_names,
        :return_type => 'CashFlowAnalysisResponse')
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ProtonApi#cash_flow_analysis\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
